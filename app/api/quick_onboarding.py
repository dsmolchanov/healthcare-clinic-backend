...
class QuickOnboardingService:
    def __init__(self):
        ...
        # Configure client to use different schemas
        ...
        self.supabase_core = create_client(
            supabase_url,
            supabase_key,
            options=ClientOptions(schema='core')
        )
        self.supabase_healthcare = create_client(
            supabase_url,
            supabase_key,
    ...
    async def quick_register(self, data: QuickRegistration) -> Dict[str, Any]:
        ...
        self.supabase_core.table('organizations').insert(org_data).execute()
        ...
        self.supabase_healthcare.table('clinics').insert(clinic_data).execute()
        ...
        self.supabase_core.table('agents').insert(agent_data).execute()
        ...
        self.supabase_healthcare.table('doctors').insert(doctor_data).execute()
        ...
        self.supabase_core.table('consent_records').insert(consent_data).execute()
    ...
    async def setup_whatsapp_simple(
        ...
        # Get clinic info
        clinic = self.supabase_healthcare.table('clinics').select('*').eq(
            'id', clinic_id
        ).single().execute()
        ...
        if data.use_shared_account:
            ...
            self.supabase_core.table('organization_secrets').insert({
                'id': str(uuid.uuid4()),
                'organization_id': clinic.data['organization_id'],
        ...
        self.supabase_core.table('whatsapp_business_configs').insert(config_data).execute()
    ...
    async def quick_calendar_setup(
        ...
        doctor = self.supabase_healthcare.table('doctors').select('id').eq(
            'clinic_id', clinic_id
        ).limit(1).execute()
    ...
    async def activate_clinic(self, clinic_id: str) -> Dict[str, Any]:
        ...
        # Get clinic info
        clinic = self.supabase_healthcare.table('clinics').select('organization_id').eq(
            'id', clinic_id
        ).single().execute()
        ...
        self.supabase_core.table('organizations').update({
            'settings': {
...
# Initialize service - lazy loading to avoid connection issues at import time
service = None
    ...
    try:
        ...
        if supabase_url != "NOT_SET":
            ...
            url_info = {
                ...
                "full_url": supabase_url[:50] + "..." if len(supabase_url) > 50 else supabase_url
            }
        else:
            url_info = {"error": "SUPABASE_URL not set"}

        svc = get_service()
        # Try a simple query
        result = svc.supabase_core.table('organizations').select('id').limit(1).execute()
        return {"status": "success", "data": result.data, "url_info": url_info}
    except Exception as e:
        import traceback
        return {
            "status": "error",
            ...
            "url_info": url_info if 'url_info' in locals() else {"error": "Failed before URL check"},
...
async def parse_website(data: WebsiteParseRequest):
    """Parse clinic information from website using AI"""
    try:
        return await get_service().parse_website(data.url)
    except Exception as e:
...
async def quick_register(data: QuickRegistration):
    ...
    try:
        return await get_service().quick_register(data)
    except Exception as e:
        import traceback
        error_details = {
            "error": str(e),
        ...
        print(f"Error in quick_register: {error_details}")
        raise HTTPException(status_code=400, detail=str(e))
...
async def setup_whatsapp(clinic_id: str, data: QuickWhatsApp):
    """Simple WhatsApp setup"""
    try:
        return await get_service().setup_whatsapp_simple(clinic_id, data)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def quick_calendar(clinic_id: str, data: QuickCalendar):
    ...
    try:
        return await get_service().quick_calendar_setup(clinic_id, data)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def activate_clinic(clinic_id: str):
    """Activate clinic"""
    try:
        return await get_service().activate_clinic(clinic_id)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
